...
func TestGenerateKeystoreFromPemKeys(t *testing.T) {
	alias := "test"
	password := "changeit"
	...
	keyBytes, err := GenerateKeyStoreFromPem(alias, password, pemCert, pemKey)
	assert.Nil(t, err)

	keyStore, err := keystore.Decode(bytes.NewReader(keyBytes),[]byte(password))
	assert.Nil(t, err)
	...
	certificate, err := x509.ParseCertificate(cert)
	assert.Nil(t, err)
	assert.Equal(t, "dv-customer.myproject.svc", certificate.Subject.CommonName)
}
...
func TestTrustStore(t *testing.T) {
	...
	keyBytes, err := GenerateTrustStoreFromPem(alias, password, defaultTrustCert)
	assert.Nil(t, err)
	...
	for _, v := range keyStore {
		c := v.(*keystore.TrustedCertificateEntry).Certificate
		_, err := x509.ParseCertificate(c.Content)
		assert.Nil(t, err)
	}
}